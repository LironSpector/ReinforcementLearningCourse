...
class SnakeEnv(gym.Env):
    ...
    def __init__(self):
        self.action_space = spaces.Discrete(4)  # There are 4 actions: up, down, left or right.
        self.frame_size_x = 200
        self.frame_size_y = 200
        self.game_window = pygame.display.set_mode((self.frame_size_x, self.frame_size_y))
        self.reset()
        self.STEP_LIMIT = 1000  # The game will end if the snake played for 1,000 steps.
        self.sleep = 0  # For computer to train, self.sleep = 0, change it when rendering for human.

    def step(self, action):
        """
        What happens when my agent performs the action on the environment.
        """
    ...
    def reset(self):
        ...
        img = array3d(display.get_surface())  # Get the pygame window as image
        img = np.swapaxes(img, 0, 1)
        return img  # returns observation
